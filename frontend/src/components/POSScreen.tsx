...
interface InventoryItem { id: number; variant: string; category: string; millName?: string; lotNumber?: string; quantity: number; sellingPrice: number; weightPerUnit?: number; unit: string; purchaseRate?: number; }
interface CartItem { id: number; inventoryItemId: number; item: string; lot: string; bags: number; weightPerBag: number; totalWeight: number; unit: string; rate: number; total: number; note: string; }
...
export default function POSScreen() {
  ...
  }, [inputBags, inputWeightPerBag, modalOpen]);
  ...
  const shareLastSaleWhatsApp = () => {
    ...
    const itemsText = (lastSale.Items || []).map((i: any) => `${i.inventoryItem?.variant || 'Item'} - ${Number(i.quantity || 0).toFixed(2)} ${i.unit || 'kg'} @ Rs ${Number(i.rate || 0).toLocaleString()} = Rs ${Number(i.total || 0).toLocaleString()}`).join('\n');
            ...
            <div className="p-5 space-y-4 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" autoFocus value={inputBags} onChange={e=>setInputBags(e.target.value)} className="w-full border-2 border-slate-200 rounded-xl p-2.5 text-lg font-black outline-none focus:border-indigo-500 transition-colors" placeholder="0" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Weight Per Bag</label>
                  <input type="number" value={inputWeightPerBag} onChange={e=>setInputWeightPerBag(e.target.value)} className="w-full border-2 border-slate-200 rounded-xl p-2.5 text-lg font-black outline-none focus:border-indigo-500 transition-colors" placeholder="0" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-stretch border-2 border-slate-200 rounded-xl overflow-hidden focus-within:border-indigo-500 transition-colors">
                  <input type="number" value={inputTotalWeight} onChange={e=>setInputTotalWeight(e.target.value)} className="w-full p-2.5 text-xl font-black outline-none" placeholder="0" />
