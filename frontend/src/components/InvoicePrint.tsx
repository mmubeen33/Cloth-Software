  ...
  return (
    <div ref={ref} className="hidden print:block bg-white text-black" style={{ fontFamily: 'Arial, sans-serif', width: '80mm', padding: '4mm', margin: '0 auto', fontSize: '11px', lineHeight: '1.35' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '8px' }}>
        ...
        <tbody>{items.map((i:any, idx:number) => <tr key={idx} style={{ borderBottom: '1px dashed #d1d5db' }}><td style={{ padding: '5px 0', fontWeight: 700 }}>{i.inventoryItem?.variant || i.item || '-'}<br/><span style={{ fontSize: '9px', fontWeight: 400 }}>Lot: {i.inventoryItem?.lotNumber || '-'} • Bags: {i.bags || 0}</span></td><td style={{ textAlign: 'right' }}>{Number(i.quantity || 0).toFixed(2)} {i.unit || 'kg'}</td><td style={{ textAlign: 'right' }}>{Number(i.rate || 0).toLocaleString()}</td><td style={{ textAlign: 'right', fontWeight: 800 }}>{Number(i.total || 0).toLocaleString()}</td></tr>)}</tbody>
      </table>
